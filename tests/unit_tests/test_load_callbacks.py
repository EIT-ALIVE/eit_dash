...
from eit_dash.callbacks.load_callbacks import load_file, open_modal, show_info
...
def test_open_modal_callback():
    context_value.set(AttributeDict(triggered_inputs=[{"prop_id": f"{ids.SELECT_FILES_BUTTON}.n_clicks"}]))
    ...
    context_value.set(AttributeDict(triggered_inputs=[{"prop_id": f"{ids.ADD_DATA_BUTTON}.n_clicks"}]))
